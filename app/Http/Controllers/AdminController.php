...
class AdminController extends Controller {
    ...
    public function manage_edit_tourlist(Request $request) {
        ...
        return view('admin.manage-edit-tourlist', compact('tourPackageDetail', 'tourPackage'));
    }
    ...
    public function updateAttraction() {
        ...
        try {
            ...
            $update_attraction_url = $_POST['update_attraction_url'];
            $attraction_picture = $_FILES['file']['name'];
            $country_id = $_POST['update_country_select'];
            $attractionModel->editAttraction($id, $country_id, $update_attraction_name, $attraction_picture, $update_attraction_url);
            echo "<script>
             alert('แก้ไขข้อมูลเสร็จสมบูรณ์');
             window.location.href='manage-attraction';  
             </script>";
...
//                }
                ...
                $conditions_id = 1;
                if (!$quick_tour) {
                    $conditions_id = $this->saveConditions(null, $request);
                }
...
//                        }
    ...
    public function saveConditions($conditions_id, $request) {
        try {
            if ($this->IsNullOrEmptyString($conditions_id)) {
                ...
                $result_id = $this->_conditions->insertConditions($conditions_name, $rate_include, $rate_not_include, $payment_condition, $cancel_change, $other_condition, $beyond_respon, $suggest_warning, $visa_detail, $agreement);
                return $result_id;
            } else {
                $conditions_name = $request->get('conditions_name');
                $rate_include = $request->get('rate_include');
