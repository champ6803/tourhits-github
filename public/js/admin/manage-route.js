$(function () {
    $('#searchButton').click(function () {
        var input_route_nname= $('#input_route_nname').val();
        var checkEmpty = input_route_nname.trim();
        if(checkEmpty.length<=0){
            refresh()
        }
        else{
             findRouteByName(input_route_nname);
        }
    });
    $('#clearButton').click(function () {
           $('#routeTable').dataTable().fnClearTable();
           $('#input_route_nname').val('');
           $('#route_name').val('');
    });
...
function findRouteByName(routeName){
        var Str = '';
            var input_route_name= $('#input_route_nname').val();
            $.ajax({
            type: 'post',
            url: 'searchRoute',
            async: false,
